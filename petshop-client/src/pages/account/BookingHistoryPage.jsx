...
export default function BookingHistoryPage() {
    ...
    const getStatusConfig = (status) => {
        const configs = {
            ...
            ASSIGNED: { label: 'Đã phân công', color: 'bg-gray-100 text-gray-800', icon: ClockIcon },
            CONFIRMED: { label: 'Đã xác nhận', color: 'bg-blue-100 text-blue-800', icon: CheckCircleIcon },
            INPROGRESS: { label: 'Đang làm', color: 'bg-orange-100 text-orange-800', icon: ClockIcon },
            ...
            REJECTED: { label: 'Bị từ chối', color: 'bg-red-100 text-red-800', icon: XCircleIcon }
        };
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="mb-8">
                    <h1 className="text-3xl font-bold text-gray-900 mb-2 flex items-center">
                        <CalendarDaysIcon className="w-8 h-8 mr-3 text-black" />
                        Lịch hẹn dịch vụ
                    </h1>
                    <p className="text-gray-600">Quản lý tất cả lịch hẹn dịch vụ của bạn</p>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="space-y-4">
                        {filteredBookings.map((booking) => {
                            ...
                            return (
                                <div key={booking.id} className="bg-white rounded-none shadow-sm hover:shadow-md transition-shadow">
                                    <div className="p-6">
                                        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <button
                                                    ...
                                                    className="flex items-center gap-2 px-4 py-2 bg-gray-100 text-black rounded-none hover:bg-gray-200 dark:hover:bg-indigo-900/30 transition-colors"
                                                >
                                                ...
                                                {canCancel && (
                                                    <button
                                                        ...
                                                        className="flex items-center gap-2 px-4 py-2 bg-red-100 text-red-700 rounded-none hover:bg-red-200 dark:hover:bg-red-900/30 transition-colors"
                                                    >
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm mb-4">
                                            <div>
                                                <p className="text-gray-500 mb-2">Dịch vụ</p>
                                                {booking.bookingItems && booking.bookingItems.length > 0 ? (
                                                    <div className="space-y-2">
                                                        {booking.bookingItems.map((item, idx) => (
                                                            <div key={item.id || idx} className="p-2 border border-gray-200 rounded-none bg-white">
                                                                ...
                                                                {item.status && (
                                                                    <span className={`inline-block mt-1 px-2 py-0.5 rounded text-xs font-medium ${
                                                                        ...
                                                                        item.status === 'InProgress' ? 'bg-orange-100 text-orange-800' :
                                            ...
                                            <div>
                                                <p className="text-gray-500 mb-2">Thú cưng</p>
                                                <div className="p-2 border border-gray-200 rounded-none bg-white">
                                            ...
                                            <div>
                                                <p className="text-gray-500 mb-2">Thông tin</p>
                                                <div className="p-2 border border-gray-200 rounded-none bg-white">
                                                    <p className="font-bold text-black text-lg">{booking.totalPrice?.toLocaleString('vi-VN') || '—'} ₫</p>
                                                    {booking.createdAt && (
                            ...
                            <div className="p-6 space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <div className="mt-1">
                                            <span className={`inline-flex items-center gap-2 px-3 py-1 rounded-none text-sm font-medium ${getStatusConfig(selectedBooking.status).color}`}>
                                    ...
                                    {selectedBooking.bookingItems && selectedBooking.bookingItems.length > 0 ? (
                                        <div className="mt-2 space-y-3">
                                            {selectedBooking.bookingItems.map((item, idx) => (
                                                <div key={item.id || idx} className="p-3 border border-gray-200 rounded-none">
                                                    <div>
                                                    ...
                                                    {item.status && (
                                                        <span className={`inline-block mt-1 px-2 py-0.5 rounded text-xs font-medium ${
                                                            ...
                                                            item.status === 'InProgress' ? 'bg-orange-100 text-orange-800' :
                                                            item.status === 'Confirmed' ? 'bg-blue-100 text-blue-800' :
                                                            item.status === 'Assigned' ? 'bg-gray-100 text-gray-800' :
                                                            item.status === 'Rejected' || item.status === 'Cancelled' ? 'bg-red-100 text-red-800' :
                                                            'bg-yellow-100 text-yellow-800'
                                                        }`}>
                                                            {item.status === 'Completed' ? 'Hoàn thành' :
                                            ...
                                            <div className="mt-3 p-3 bg-white rounded-none">
                                ...
                                {selectedBooking.note && (
                                    <div>
                                        ...
                                        <p className="mt-1 p-3 bg-white rounded-none">{selectedBooking.note}</p>
                                    </div>
                                )}
                                ...
                                {selectedBooking.internalNote && (
                                    <div>
                                        ...
                                        <p className="mt-1 p-3 bg-blue-50 rounded-none">{selectedBooking.internalNote}</p>
                                    </div>
                                )}
