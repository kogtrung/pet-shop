...
HÃY SỬ DỤNG TOOL call_get_api VỚI CÁC ĐƯỜNG DẪN TRÊN khi cần lấy danh sách hoặc tìm kiếm. Để lấy chi tiết 1 sản phẩm đã biết ID, hãy dùng tool get_product_context. Để tóm tắt bài viết đã biết slug, hãy dùng tool get_article_content_by_slug. Đừng trả lời 'tôi không biết' nếu bạn có thể dùng tool để lấy thông tin. ---KẾT THÚC HƯỚNG DẪN--- " };
    ...
    private async Task<string> ProcessQueryWithGeminiAndTools(string prompt, List<ChatMessage> conversationMessages, string? clientBaseUrl)
    {
        ...
        catch (Exception ex)
        {
            ...
            // Xử lý báo lỗi thân thiện cho UI frontend thay vì in toàn bộ JSON
            string errorStr = ex.Message + " " + ex.InnerException?.Message;
            if (errorStr.Contains("503") || errorStr.Contains("ServiceUnavailable") || errorStr.Contains("high demand"))
            {
                return "Xin lỗi bạn, hiện tại máy chủ AI của Google Gemini đang bị quá tải cục bộ. Bạn vui lòng đợi một lát rồi thử lại nhé!";
            }
            if (errorStr.Contains("429") || errorStr.Contains("Too Many Requests"))
            {
                return "Xin lỗi, tổng lượt nhắn tin đã đạt giới hạn an toàn. Bạn vui lòng đợi một chút rồi chat tiếp nhé!";
            }

            // Vẫn trả về thông báo lỗi chung cho người dùng cuối
            return "Xin lỗi, hệ thống AI của tôi đang gặp trục trặc kỹ thuật do lỗi mạng. Vui lòng thử lại sau.";
        }
    }
